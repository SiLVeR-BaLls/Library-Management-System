...
<div class="flex min-h-screen">
    ...
    <div class="flex flex-col flex-1">
        ...
        <!-- BorrowBook Content -->
        <div class="flex flex-1 justify-center items-start pt-12"> <!-- Center horizontally, align to top with padding -->
            <div class="container max-w-2xl w-full px-4 py-6 flex flex-col items-center justify-start">
                <h2 class="text-3xl font-semibold mb-6 text-center">Borrow a Book</h2>

                <form action="include/BorrowConnect.php" method="POST" class="w-full">
                    <div class="flex flex-row gap-4 mt-2">
                            ...
                            <div class="flex flex-col space-y-2">
                                <div class="flex">
                                    ...
                                    <button type="button" class="ml-2 bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-700"
                                        onclick="startScanner('IDno')">Scan QR
                                    </button>
                                </div>
                            ...
                            <div class="form-group flex flex-col space-y-2" id="bookGroup_0">
                                ...
                                <div class="flex flex-col space-y-2">
                                    <div class="flex">
                                        ...
                                        <button type="button" class="ml-2 bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-700"
                                            onclick="startScanner('bookID_0')">Scan QR
                                        </button>
                                    </div>
                        ...
                        <div id="scannerModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 hidden">
                            <div class="bg-white p-4 rounded-lg shadow-lg relative">
                                ...
                                <button type="button" onclick="stopScanner()" class="absolute top-2 right-2 bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-700">Close
                                </button>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function searchBook(index) {
        ...
        if (bookID.length > 2) {
            fetch(`page/include/SearchBook.php?bookID=${bookID}`)
                .then(response => response.text())
                .then(data => {
...
<script>
    function searchUser() {
        const IDno = document.getElementById('IDno').value;
        if (IDno.length > 0) {
            fetch(`page/include/SearchUser.php?IDno=${IDno}`)
                .then(response => response.text())
                .then(data => {
                    document.getElementById('userSearchResult').innerHTML = data;
                    const results = document.getElementById('userSearchResult').getElementsByClassName('search-item');
